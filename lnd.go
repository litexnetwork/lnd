...
func lndMain() error {
	...
	if cfg.Router.MultiPathRouter {
		...
		server.multiPathRouter.AddInvoice = func(amount btcutil.Amount) ([]byte, error) {
			var paymentPreimage [32]byte
			if _, err := rand.Read(paymentPreimage[:]); err != nil {
				return nil, err
			}
			amtMSat := lnwire.NewMSatFromSatoshis(amount)
			...
			options = append(options, zpay32.Description("auto generated by multipayment"))
			...
			payReqString, err := payReq.Encode(
				zpay32.MessageSigner{
					SignCompact: server.nodeSigner.SignDigestCompact,
				},
			)
			if err != nil {
				return nil, err
			...
			i := &channeldb.Invoice{
				...
				Type: int32(channeldb.NORMAL_INVOICE),
			}
			copy(i.Terms.PaymentPreimage[:], paymentPreimage[:])
			if err := server.invoices.AddInvoice(i); err != nil {
				return nil, err
			}
			return []byte(payReqString), nil
		}
